...
class Tribulle:
    try:
        ...
        def __init__(this, player, server):
            ...
            this.TRIBE_RANKS = "0|${trad#TG_0}|0;0|${trad#TG_1}|0;2|${trad#TG_2}|0;3|${trad#TG_3}|0;4|${trad#TG_4}|32;5|${trad#TG_5}|160;6|${trad#TG_6}|416;7|${trad#TG_7}|932;8|${trad#TG_8}|2044;9|${trad#TG_9}|2046"
        ...
        def sendFriendsList(this, readPacket):
            ...
            if this.client.marriage == "":
                ...
                this.Cursor.execute("select Username, PlayerID, Gender, LastOn from Users where Username = ?", [this.client.marriage])
                rs = this.Cursor.fetchone()
                player = this.server.players.get(this.client.marriage)
                p.writeInt(rs[1]).writeUTF(rs[0].lower()).writeByte(rs[2]).writeInt(rs[1]).writeByte(1).writeBoolean(this.server.checkConnectedAccount(rs[0])).writeInt(4).writeUTF(player.roomName if player else "").writeInt(rs[3])
            infos = {}
        ...
        def addFriend(this, readPacket):
            ...
            this.Cursor.execute("select Username, PlayerID, Gender, LastOn from Users where Username = ?", [playerName])
            rs = this.Cursor.fetchone()
            if not this.server.checkExistingUser(playerName):
                ...
                this.sendPacket(36, ByteArray().writeInt(rs[1]).writeUTF(Utils.parsePlayerName(playerName)).writeByte(rs[2]).writeInt(rs[1]).writeShort(this.server.checkConnectedAccount(playerName)).writeInt(4 if isFriend else 0).writeUTF(player.roomName if isFriend and player != None else "").writeInt(rs[3] if isFriend else 0).toByteArray())
        ...
        def sendFriendChangedRoom(this, playerName, langueID):
            if playerName in this.client.friendsList:
                player = this.server.players.get(playerName)
                this.sendPacket(35, ByteArray().writeInt(player.playerID).writeUTF(playerName.lower()).writeByte(player.gender).writeInt(player.playerID).writeByte(1).writeByte(1).writeInt(4).writeUTF(player.roomName).writeInt(player.lastOn).toByteArray())
                    
        def sendFriendDisconnected(this, playerName):
            if playerName in this.client.friendsList:
                this.Cursor.execute("select Username, PlayerID, Gender, LastOn from Users where Username = ?", [playerName])
                rs = this.Cursor.fetchone()
                this.sendPacket(35, ByteArray().writeInt(rs[1]).writeUTF(playerName.lower()).writeByte(rs[2]).writeInt(rs[1]).writeByte(1).writeByte(0).writeInt(1).writeUTF("").writeInt(rs[3]).toByteArray())
        ...
        def whisperMessage(this, readPacket):
            ...
            isCheck = this.server.checkMessage(message)
        ...
        def disableWhispers(this, readPacket):
            ...
            this.client.silenceMessage = "" if this.server.checkMessage(message) else message
        ...
        def createTribe(this, readPacket):
            ...
            if not this.checkExistingTribe(tribeName):
                if this.client.shopCheeses >= 500:
                    this.Cursor.execute("insert into Tribe  values(null, ?, '', '0', ?, '', ?, ?)", [tribeName, this.TRIBE_RANKS, this.client.playerName, this.server.lastChatID])
        ...
        def tribeInvite(this, readPacket):
            ...
            if not this.server.checkConnectedAccount(playerName) or not this.server.checkExistingUser(playerName):
                this.sendPacket(79, packet.writeByte(11).toByteArray())
            elif not player.tribeName == "":
                this.sendPacket(79, packet.writeByte(17).toByteArray())
            else:
            ...
            for playerName in members:
                ...
                if player != None:
                    if player.tribeRank >= 1:
                        player.tribeRank += 1
                        this.Cursor.execute("update users set TribeRank = ? where Username = ?", [tribeRank+1, playerName])
                else:
                    if tribeRank >= 1:
                        this.Cursor.execute("update users set TribeRank = ? where Username = ?", [tribeRank+1, playerName])
        ...
        def deleteTribeRank(this, readPacket):
            ...
            for playerName in members:
                ...
                if player != None:
                    ...
                    tribeRank = this.getPlayerTribeRank(playerName)
                    if tribeRank == rankID:
                        this.Cursor.execute("update users set TribeRank = 0 where Username = ?", [playerName])
                    else:
                        continue
            for playerName in members:
                player = this.server.players.get(playerName)
            ...
            for member in members:
                ...
                if player != None:
                    if player.tribeRank == rankID:
                        ...
                        this.Cursor.execute("update users set TribeRank = ? where Username = ?", [rankID2, member])
                    if up:
                        if player.tribeRank == rankID2:
                            player.tribeRank -= 1
                            this.Cursor.execute("update users set TribeRank = ? where Username = ?", [rankID2 - 1, member])
                    if down:
                        if player.tribeRank == rankID2:
                            player.tribeRank += 1
                            this.Cursor.execute("update users set TribeRank = ? where Username = ?", [rankID2 + 1, member])
                else:   
                    this.Cursor.execute("select TribeRank from users where Username = ?", [member])
                    rankPlayer = this.Cursor.fetchone()[0]
        ...
        def changeTribePlayerRank(this, packet):
            ...
            if player != None:
                ...
                this.Cursor.execute("update users set TribeRank = ? where Username = ?", [rankID, playerName])
            else:
                this.Cursor.execute("update users set TribeRank = ? where Username = ?", [rankID, playerName])
            ...
            if tribeCode != 0:
                ...
                if playerName in members:
                    ...
                    if player != None:
                        ...
                        this.Cursor.execute("update users set TribeCode = 0, TribeRank = 0, TribeJoined = 0 where Username = ?", [playerName])
                members = this.getTribeMembers(this.client.tribeCode)
                for member in members:
                    player = this.server.players.get(member)
                    if player != None:
        ...
        def setTribeMaster(this, packet):
            ...
            this.Cursor.execute("update users set TribeRank = ? where Username = ?", [len(rankInfo)-2, this.client.playerName])
            player = this.server.players.get(playerName)
            if player != None:
                player.tribeRank = (len(rankInfo)-1)
                this.Cursor.execute("update users set TribeRank = ? where Username = ?", [len(rankInfo)-1, playerName])
            else:
                this.Cursor.execute("update users set TribeRank = ? where Username = ?", [len(rankInfo)-1, playerName])
            this.Cursor.execute("select Username, PlayerID, Gender, LastOn from Users where Username = ?", [playerName])
            rs = this.Cursor.fetchone()
        ...
        def finishTribe(this, packet):
            ...
            members = this.getTribeMembers(this.client.tribeCode)
            this.Cursor.execute("update users set TribeCode = 0, TribeRank = 0, TribeJoined = 0 where TribeCode = ?", [this.client.tribeCode])
          ...
          #  this.client.sendPacket([6, 9], ByteArray().writeUTF("A tribo foi desfeita.").toByteArray())
        ...
        def customChat(this, packet):
            ...
            if re.match("^(?=^(?:(?!.*_$).)*$)(?=^(?:(?!_{2,}).)*$)[A-Za-z][A-Za-z0-9_]{2,11}$", chatName):
                ...
                if chatID == -1:
                    this.Cursor.execute("insert into Chats values (null, ?)", [chatName])

                chatID = this.getChatID(chatName)

                this.client.chats.append(chatID)
                this.sendPacket(62, ByteArray().writeUTF(chatName).toByteArray())
                this.sendPacket(55, ByteArray().writeInt(tribulleID).writeByte(0).toByteArray())
            else:
        ...
        def chatMembersList(this, packet):
            tribulleID, chatName = packet.readInt(), packet.readUTF()
            p = ByteArray().writeInt(tribulleID).writeByte(0)
            chatID = this.getChatID(chatName)
            length = 0
            for player in this.server.players.values():
                if chatID in player.chats:
        ...
        def getUserFriends(this, playerName):
            this.Cursor.execute("select FriendsList from Users where Username = ?", [playerName])
            rs = this.Cursor.fetchone()
            if rs:
        ...
        def getPlayerTribeRank(this, playerName):
            this.Cursor.execute("select TribeRank from users where Username = ?", [playerName])
            rs = this.Cursor.fetchone()
            if rs:
                return rs["TribeRank"]
            else:
        ...
        def updateTribeRanks(this):
            this.Cursor.execute("update tribe set Ranks = ? where Code = ?", [this.client.tribeRanks, this.client.tribeCode])
        ...
        def getPlayerTribeCode(this, playerName):
            this.Cursor.execute("select TribeCode from users where Username = ?", [playerName])
            rs = this.Cursor.fetchone()
            if rs:
                return rs["TribeCode"]
            else:
